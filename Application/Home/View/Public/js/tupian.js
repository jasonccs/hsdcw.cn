...
(function($){
    ...
    $("div[data-scro='tupian'] b,div[data-scro='tupian2'] a").click(function(){
        var T = $(this);
        if(T.attr("class")=="down") return false;
        J2ROLLING_ANIMATION.st({
            findObject : T, //当前点击对象 默认写
            main : T.parent().parent().find("div[data-scro='listtupian']"),   //滚动目标容器窗口对象
            pagSource : T.parent().parent().find("div[data-scro='tupian'] b"),   //切换按钮对象
    ...
    var J2SETTIME="", J2Time=true,J2ROLLING_ANIMATION = {
        ...
        start : function(){
            $("#section-focus-pic div[data-scro='tupian'] b,#section-focus-pic div[data-scro='tupian2'] a").mouseover(function(){
        ...
        time : function(){
            J2SETTIME = window.setInterval(function(){
                var num = $("#section-focus-pic div[data-scro='tupian'] b[class='down']").index(),
                    _list = $("#section-focus-pic div[data-scro='listtupian']>ul>.item");
                _list.eq(num).animate({"left":-$("#section-focus-pic div[data-scro='listtupian']").width()},"slow",function(){
                    $(this).removeAttr("style");    
                    $("#section-focus-pic div[data-scro='tupian'] b").removeClass("down").eq(num).addClass("down");
                }); 
                num++;
                if(num==_list.size()){
                    num=0;
